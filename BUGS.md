...
### [2026-05-22] ШІ після мату встановлює результат "Нічия" замість "Перемога/Поразка"

- **Контекст:** `server/src/socket/handlers/game.handler.ts` — блок обробки ходу ШІ (~рядок 118)
- **Симптом:** Після того як ШІ давав шах-і-мат гравцю (або гравець давав мат ШІ через ходи без участі AI-блоку) — гра завершувалась з результатом "Нічия" замість відповідної перемоги/поразки
- **Причина:** Код AI-блоку мав `const result = updatedChess.isCheckmate() ? 'BLACK_WIN' : 'DRAW'` — HARDCODED `'BLACK_WIN'`. Це не враховувало ситуацію де ШІ може грати білими або інші edge-cases. Додатково: `chess.isGameOver()` повертає `true` для МАТ + СТЕЙЛМАТ + НІЧИЯ — тому при будь-якому завершенні крім мату результат падав на `'DRAW'` навіть якщо було якесь інше завершення
- **Рішення:** Замінено hardcoded `'BLACK_WIN'` на dynamic: `updatedChess.turn() === 'w' ? 'BLACK_WIN' : 'WHITE_WIN'`. `chess.turn()` після ходу повертає сторону яка ПОВИННА ходити але не може — це програвша сторона при маті. Аналогічне виправлення застосовано і до блоку ходу гравця для консистентності
- **Як уникнути:** Ніколи не хардкодити результат гри в логіку яка може виконуватись для обох сторін. Завжди використовувати `chess.turn()` для визначення програвшої сторони при маті — це єдине надійне джерело правди в chess.js

---

### [2026-05-22] Мітки складності ШІ в лобі не відповідали реальним глибинам пошуку

- **Контекст:** `client/src/pages/LobbyPage.tsx`
- **Симптом:** UI показував "800–1000 ELO · глибина 2/3/4" але реальні глибини після попереднього рефакторингу стали 1/2/3. Гравець отримував некоректну інформацію про складність
- **Причина:** Масив `AI_LEVELS` у `LobbyPage.tsx` не оновлювався синхронно з кодом `aiService.ts` при зміні глибин
- **Рішення:** Оновлено масив: EASY → "Випадкові помилки · глибина 1", MEDIUM → "Стандартний · глибина 2", HARD → "Сильний · глибина 3". Замінено поле `ratingRange` на `description` яке описує поведінку а не умовний рейтинг
- **Як уникнути:** Параметри ШІ (depth, behavior) мають бути в одному місці — ideally константи з спільного модуля або config. При зміні `aiService.ts` — обов'язково перевіряти UI labels
